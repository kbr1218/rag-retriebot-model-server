...
def find_user_vectors(user_id: str, vectorstore: FAISS, embeddings: Embeddings) -> List[Tuple[Document, float]]:
  """
  user_id에 해당하는 벡터들을 벡터스토어에서 검색하는 함수

  Args:
      user_id (str): 검색할 사용자 ID.
      vectorstore (FAISS): FAISS 벡터스토어 객체
  ...
  try:
    ...
    search_results = vectorstore.similarity_search_with_score_by_vector(query_embedding, k=vectorstore.index.ntotal)
